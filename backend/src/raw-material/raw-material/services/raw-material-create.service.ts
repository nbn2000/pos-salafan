...
export class RawMaterialCreateService extends RawMaterialBaseService {
  async createWithBatchAndFinance(
    ...
    if (!actorUserId)
      throw new BadRequestException('Foydalanuvchi IDsi talab qilinadi');
    assertUuid(actorUserId, 'userId (token)');

    const rawBatchUnknown: unknown =
      typeof dto.batch === 'string' ? safeParseJson(dto.batch, 'batch') : dto.batch;
    ...
    if (!isFinite(batch.amount) || batch.amount <= 0)
      throw new BadRequestException('Batch miqdori 0 dan katta bo‘lishi kerak');
    if (!isFinite(batch.buyPrice) || batch.buyPrice <= 0)
      throw new BadRequestException('Olish narxi 0 dan katta bo‘lishi kerak');
    if (paid < 0)
    ...
    if (paid > result)
      throw new BadRequestException(
        'To‘langan summa umumiy xarajatdan oshib ketgan',
      );
    if (paid > 0 && !dto.paymentType)
      throw new BadRequestException(
        'To‘lov turi talab qilinadi (paid > 0)'
      );
    ...
    const savedRaw = await this.repo.manager.transaction(async (em) => {
      ...
      const comment =
        `${rawSaved.name} nomli xomashyo yaratildi va birinchi partiya qo‘shildi: ` +
      ...
      const logSaved = await logRepo.save(
        logRepo.create({
          ...
          comment,
          type: RawMaterialLogType.ADD,
        }),
      );

      const debtAmount = result - paid;
      const pType: PaymentType | undefined = dto.paymentType;
      ...
      if (paid > 0) {
        await paymentRepo.save(
          paymentRepo.create({
            ...
            from: actorUserId,
            to: dto.supplierId,
            comment: `Xomashyo "${rawSaved.name}" uchun to‘lov, miqdor=${batch.amount}, umumiy: ${result}`,
            transactionId: null,
